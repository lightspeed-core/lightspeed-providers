...
Audit Dependabot vulnerabilities for `$repo` (default: `lightspeed-core/lightspeed-providers`) and cross-reference them against existing LCORE Jira tickets.
...
If `$repo` is not provided, default to `lightspeed-core/lightspeed-providers`. Deduplicate results by (CVE + package name) when a CVE is present, or (GHSA ID + package name) when CVE is null/N/A. This prevents collapsing distinct GHSA-only advisories for the same package into a single entry.
...
Search for the parent epic: `project = LCORE AND issuetype = Epic AND summary ~ "CVE" AND summary ~ "lightspeed-providers" ORDER BY created DESC`. Pick the one whose fix version matches the user's chosen fix version. If ambiguous or none found, ask the user.
