...
# from llama_stack.providers.datatypes import Api, VectorStoresProtocolPrivate
...
# VERSION = "v1"
# VECTOR_STORES_PREFIX = f"vector_stores:solr:{VERSION}::"
...
#                 #TODO: replace with filter reference from run.yaml
...
# class SolrVectorIOAdapter(OpenAIVectorStoreMixin, VectorIO, VectorStoresProtocolPrivate):
#     """
...
#             start_key = VECTOR_STORES_PREFIX
#             end_key = f"{VECTOR_STORES_PREFIX}\xff"
#             stored_vector_stores = await self.kvstore.values_in_range(start_key, end_key)

#             log.info(
#                 f"Loading {len(stored_vector_stores)} persisted vector stores from KV store"
#             )
...
#         if self.kvstore is not None:
#             key = f"{VECTOR_STORES_PREFIX}{vector_store.identifier}"
...
#     async def unregister_vector_store(self, vector_db_id: str) -> None:
#         """Unregister a vector store (removes from cache and KV store)."""
#         log.info(f"Unregistering vector store: {vector_db_id}")

#         if vector_db_id in self.cache:
#             del self.cache[vector_db_id]
#             log.debug(f"Removed vector store from cache: {vector_db_id}")

#         if self.kvstore is not None:
#             await self.kvstore.delete(key=f"{VECTOR_STORES_PREFIX}{vector_db_id}")
#             log.debug("Removed from KV store")

#         log.info(f"Successfully unregistered vector store: {vector_db_id}")

#     # async def insert_chunks(
#     #     self, vector_db_id: str, chunks: list[Chunk],
#     # ) -> None:
#     #     """Not implemented - this is a read-only provider."""
#     #     log.warning(
...
#     ) -> None:
#         """Read-only provider – inserts are not supported."""
#         log.warning(
#             f"Attempted to insert {len(chunks)} chunks into read-only provider "
#             f"(vector_db_id={vector_db_id})"
#         )
...
#         log.info("Query endpoint getting hit")
...
#                 index.vector_store.embedding_model, [query_string]
#             )
...
#     ) -> VectorStoreWithIndex:
#         if vector_db_id in self.cache:
#             log.debug(f"Retrieved vector store from cache: {vector_db_id}")
...
#         if self.vector_store_store is not None:
#             vector_store = await self.vector_store_store.get_vector_store(vector_db_id)
#             if not vector_store:
#                 log.error(f"Vector DB not found in store: {vector_db_id}")
#                 raise VectorStoreNotFoundError(vector_db_id)
#             log.info(f"Loaded vector store from store: {vector_db_id}")
#         else:
#             # Fallback: vector_store_store is None, try to recreate from registered resources
#             log.debug(f"Vector store store not available, creating vector store object for: {vector_db_id}")
#             # Import the VectorStore class to create a minimal vector store object
#             from llama_stack.apis.vector_stores import VectorStore
            
#             # Create a basic vector store object with the expected ID
#             # This works for read-only scenarios since we're just using it for indexing
#             vector_store = VectorStore(
...
#             )
#             log.info(f"Created fallback vector store object: {vector_db_id}")
...
#         )
#         await index.initialize()
#         self.cache[vector_db_id] = VectorStoreWithIndex(
#             vector_store, index, self.inference_api
#         )
#         return self.cache[vector_db_id]
...
class SolrVectorIOAdapter(
    ...
    async def _get_and_cache_vector_store_index(
        ...
        log.debug(f"Vector store not in cache, loading from table: {vector_db_id}")

        if self.vector_store_table is None:
            log.error(f"Vector store table not set, cannot find: {vector_db_id}")
            raise VectorStoreNotFoundError(vector_db_id)

        vector_store = await self.vector_store_table.get_vector_store(vector_db_id)
        if not vector_store:
            log.error(f"Vector store not found: {vector_db_id}")
            raise VectorStoreNotFoundError(vector_db_id)

        log.info(f"Loaded vector store from table: {vector_db_id}")
        index = SolrIndex(
            vector_store=vector_store,
            solr_url=self.config.solr_url,
            collection_name=self.config.collection_name,
