...
# Solr query: fq=(is_chunk:true AND product:"ansible_automation_platform")
```
...
# Solr query: fq=(is_chunk:true AND product:("openshift_container_platform" OR "ansible_automation_platform" OR "rhel"))
```
...
response = await adapter.query_chunks(
    ...
    params={
        ...
        "filters": CompoundFilter(
            type="and",
            filters=[
                ComparisonFilter(type="eq", key="product", value="openshift_container_platform"),
                ComparisonFilter(type="eq", key="version", value="4.21")
            ]
        )
    }
)
# Solr query: fq=(is_chunk:true AND (product:"openshift_container_platform" AND version:"4.21"))
```
        ...
        "filters": CompoundFilter(
            ...
            filters=[
                ...
                CompoundFilter(
                    ...
                    filters=[
                        ComparisonFilter(type="eq", key="product", value="openshift_container_platform"),
                        ComparisonFilter(type="eq", key="product", value="ansible_automation_platform")
                    ]
                )
            ]
        )
...
# Solr query: fq=(is_chunk:true AND (doc_type:"guide" AND (product:"openshift_container_platform" OR product:"ansible_automation_platform")))
```
